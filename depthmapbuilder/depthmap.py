...
def make_connection():
	return pymysql.connect(host=db_host, user=db_user, password=db_password, db=db_name, charset='utf8mb4', cursorclass=DictCursor)
...
def autocrop_and_detect(imgname):
	...
	for template in templates:
		...
		if isEast:
			if isSouth:
				top_left = sorted(points, key=lambda point: point_distance((img_w, img_h), (point[0] + w, point[1] + h)))[0]
			else:
	...
	if isRect:
		for corner, side in zip(bounds, sides):
			...
			w = xBottom - xTop
			h = yBottom - yTop
			#extend bounds according to side
			if isEast:
				xTop -= w
			else:
				xBottom += w
			...
			imageCropped = cv2.adaptiveThreshold(imageCropped, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 7, 15)
			...
			if lines is not None:
				...
				cv2.circle(imageCropped, tuple(closestPoint), radius=4, color=128, thickness=-1)
				x0, y0 = closestPoint
				...
				cv2.rectangle(imageCropped, fillTop, fillBottom, 255, thickness=cv2.FILLED)
				txt = pytesseract.image_to_string(imageCropped, config=custom_config, lang='eng')
			...
			for image in images_parsed:
				...
				if x >= west_limit and x <= east_limit and y >= north_limit and y <= south_limit:
					...
					if paste_h > 0 and paste_w > 0:
						...
						crop_x = int((clamp((clamp(west_fix, tile_bound_x, tile_bound_x + tile_size - 1) - west_fix) * scale_fact_w, 0, w - 1)))
						crop_y = int((clamp((clamp(north_fix,tile_bound_y, tile_bound_y + tile_size - 1) - north_fix) * scale_fact_h, 0, h - 1)))
						crop_x_end = int((clamp((clamp(east_fix, tile_bound_x, tile_bound_x + tile_size - 1) - west_fix) * scale_fact_w, 0, w - 1)))
						crop_y_end = int((clamp((clamp(south_fix, tile_bound_y, tile_bound_y + tile_size - 1) - north_fix) * scale_fact_h, 0, h - 1)))
						...
						paste_h = int(paste_h)
						paste_w = int(paste_w)
						image_source_array = image_source.resize((paste_w, paste_h), resample=Image.BICUBIC, box=(crop_x, crop_y, crop_x_end, crop_y_end))
...
WHERE id = {8}""".format(point_nwx, point_nwy, point_nex, point_ney, point_sex, point_sey, point_swx, point_swy, image['id'])
				connection = make_connection()
				cursor = connection.cursor()
				cursor.execute(query)
				connection.commit()
				...
				if not coords_ok:
					...
					sheet_code = ''
					query = "INSERT INTO image_problem (image_id, north, south, west, east) VALUES ({0}, '{1}', '{2}', '{3}', '{4}')".format(image['id'], coord_north, coord_south, coord_west, coord_east)
					connection = make_connection()
					cursor = connection.cursor()
					cursor.execute(query)
					connection.commit()
					...
					sheet_code = get_sheet_code(parse_angle_dms(coord_north), parse_angle_dms(coord_south), parse_angle_dms(coord_west), parse_angle_dms(coord_east))
...
"""
		...
		for image in images:
			print('Processing ', image['image_path'], ' (#', image['id'], ')')
			coords = [(image['point_nw_x'], image['point_nw_y']), (image['point_ne_x'], image['point_ne_y']), (image['point_se_x'], image['point_se_y']), (image['point_sw_x'], image['point_sw_y'])]
			...
			connection.commit()
			connection.close()
			sheet_code = get_sheet_code(parse_angle_dms(image['north']), parse_angle_dms(image['south']), parse_angle_dms(image['west']), parse_angle_dms(image['east']))
			if not sheet_code is None:
				connection = make_connection()
				cursor = connection.cursor()
				query = "UPDATE images SET sheet_code = '{0}' WHERE id = {1}".format(sheet_code.replace("'", "\\'").replace('"', '\\"'), image['id'])
				cursor.execute(query)
				connection.commit()
				connection.close()
			print('Done.')
